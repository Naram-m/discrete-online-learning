...
o_ftpl_agents = [O_FTPLAgent(files=files, max_cache_size=max_cache_size, acc=0.8) for i in range(trials)]

ftrl_agents = [FTRLAgent(files=files, max_cache_size=max_cache_size) for i in range(trials)]
o_ftrl_agents = [O_FTRLAgent(files=files, max_cache_size=max_cache_size, acc=0.8) for i in range(trials)]
...
with open('P8/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('P8/FTPL.npy', 'wb') as f:
    np.save(f, util_FTPL)

with open('P8/O_FTPL_8.npy', 'wb') as f:
    np.save(f, util_O_FTPL)


with open('R8/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('R8/FTRL.npy', 'wb') as f:
    np.save(f, util_FTRL)

with open('R8/O_FTRL_8.npy', 'wb') as f:

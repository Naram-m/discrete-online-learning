...
o_ftpl_agents = [O_FTPLAgent(files=files, max_cache_size=max_cache_size, acc=0.4) for i in range(trials)]

ftrl_agents = [FTRLAgent(files=files, max_cache_size=max_cache_size) for i in range(trials)]
o_ftrl_agents = [O_FTRLAgent(files=files, max_cache_size=max_cache_size, acc=0.4) for i in range(trials)]
...
with open('P4/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('P4/FTPL.npy', 'wb') as f:
    np.save(f, util_FTPL)

with open('P4/O_FTPL_4.npy', 'wb') as f:
    np.save(f, util_O_FTPL)


with open('R4/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('R4/FTRL.npy', 'wb') as f:
    np.save(f, util_FTRL)

with open('R4/O_FTRL_4.npy', 'wb') as f:

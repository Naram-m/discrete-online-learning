...
o_ftpl_agents = [O_FTPLAgent(files=files, max_cache_size=max_cache_size, acc=0.01) for i in range(trials)]

ftrl_agents = [FTRLAgent(files=files, max_cache_size=max_cache_size) for i in range(1)]
o_ftrl_agents = [O_FTRLAgent(files=files, max_cache_size=max_cache_size, acc=0.01) for i in range(trials)]
...
with open('P01/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('P01/FTPL.npy', 'wb') as f:
    np.save(f, util_FTPL)

with open('P01/O_FTPL_01.npy', 'wb') as f:
    np.save(f, util_O_FTPL)


with open('R01/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('R01/FTRL.npy', 'wb') as f:
    np.save(f, util_FTRL)

with open('R01/O_FTRL_01.npy', 'wb') as f:

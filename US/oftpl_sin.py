...
class O_FTPLAgent_SIN():
    def __init__(self, files, max_cache_size, acc):
        ...
        self.const = 1 / np.sqrt(self.max_cache_size) * (1 / np.log(self.files * np.exp(1) / self.max_cache_size)) ** (
                    1 / 4)
    ...
    def step(self, grad):
        ...
        self.t += 1
        prob_correct = 0.2 * math.sin(self.t * math.pi / 1000) + 0.75

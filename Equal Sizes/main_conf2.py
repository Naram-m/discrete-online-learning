...
o_ftpl_agents = [O_FTPLAgent(files=files, max_cache_size=max_cache_size, acc=0.75) for i in range(trials)]
o_ftpl_agents_0 = [O_FTPLAgent(files=files, max_cache_size=max_cache_size, acc=0.0) for i in range(trials)]
o_ftpl_agents_sin = [O_FTPLAgent_SIN(files=files, max_cache_size=max_cache_size, acc=-1) for i in range(trials)]


# ftrl_agents = [FTRLAgent(files=files, max_cache_size=max_cache_size) for i in range(trials)]
# o_ftrl_agents = [O_FTRLAgent(files=files, max_cache_size=max_cache_size, acc=0.75) for i in range(trials)]
# o_ftrl_agents_0 = [O_FTRLAgent(files=files, max_cache_size=max_cache_size, acc=0.0) for i in range(trials)]
# o_ftrl_agents_sin = [O_FTRLAgent_SIN(files=files, max_cache_size=max_cache_size, acc=-1) for i in range(trials)]
...
for t in np.arange(1, T + 1):
    ...
    ################ FTPL sin #################
    for agent_index, o_ftpl_agent_sin in enumerate(o_ftpl_agents_sin):
        y_o_ftpl_sin = o_ftpl_agent_sin.step(c_t)
        oftpl_agents_sin_utils.append(c_t @ y_o_ftpl_sin)
    ...
    if np.isnan(a) or np.isnan(b):
        a = avg
        b = avg
    util_O_FTPL_sin.append([avg, a, b])
    ...
    ################ FTRL sin #################
    # for agent_index, o_ftrl_agent_sin in enumerate(o_ftrl_agents_sin):
    #     y_o_ftrl_sin = o_ftrl_agent_sin.step(c_t)
    #     oftrl_agents_sin_utils.append(c_t @ y_o_ftrl_sin)
    ...
    #     a = avg
    #     b = avg
    # util_O_FTRL_sin.append([avg, a, b])
...
util_O_FTPL_sin = np.array(util_O_FTPL_sin)
util_O_FTPL_sin = np.cumsum(util_O_FTPL_sin, 0)
...
util_O_FTRL_sin = np.array(util_O_FTRL_sin)
util_O_FTRL_sin = np.cumsum(util_O_FTRL_sin, 0)




with open('./results2/BH.npy', 'wb') as f:
    np.save(f, util_BHS)

with open('./results2/FTPL.npy', 'wb') as f:
    np.save(f, util_FTPL)

with open('./results2/O_FTPL.npy', 'wb') as f:
    np.save(f, util_O_FTPL)

with open('./results2/O_FTPL_0.npy', 'wb') as f:
    np.save(f, util_O_FTPL_0)

with open('./results2/O_FTPL_sin.npy', 'wb') as f:
    np.save(f, util_O_FTPL_sin)

# with open('./results2/FTRL.npy', 'wb') as f:
#     np.save(f, util_FTRL)
#
# with open('./results2/O_FTRL.npy', 'wb') as f:
#     np.save(f, util_O_FTRL)
#
# with open('./results2/O_FTRL_0.npy', 'wb') as f:
#     np.save(f, util_O_FTRL_0)
#
# with open('./results2/O_FTRL_sin.npy', 'wb') as f:
#     np.save(f, util_O_FTRL_sin)
